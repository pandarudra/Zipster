...
async function Module(moduleArg = {}) {
  ...
  // ENVIRONMENT_IS_NODE.
    ...
    try {
      // Fix for Next.js bundled environment - handle URL resolution more safely
      if (
        typeof _scriptName === "string" &&
        _scriptName.startsWith("file://")
      ) {
        scriptDirectory = new URL(".", _scriptName).href; // includes trailing slash
      } else {
        // For bundled environments, use root or empty
        scriptDirectory = "/";
      }
    } catch {
      // Must be a `blob:` or `data:` URL (e.g. `blob:http://site.com/etc/etc`), we cannot
      // infer anything from them.
      // For bundled environments like Next.js, use empty string or root
      scriptDirectory = "";
    }
...
/** not-@type {!BigUint64Array} */
  ...
  function findWasmBinary() {
    if (Module["locateFile"]) {
      return locateFile("huffman.wasm");
    }
    // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
    return new URL("huffman.wasm", import.meta.url).href;
  }
  ...
  var _compress = (Module["_compress"] = makeInvalidEarlyAccess("_compress"));
  var _decompress = (Module["_decompress"] =
    makeInvalidEarlyAccess("_decompress"));
  ...
  function assignWasmExports(wasmExports) {
    ...
    Module["_compress"] = _compress = createExportWrapper("compress", 3);
    Module["_decompress"] = _decompress = createExportWrapper("decompress", 3);
